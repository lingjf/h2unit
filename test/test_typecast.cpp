...
#define TheCheck(x) static void foo2##x(decltype(x) v){};
ForForEach(TheCheck, PTR_LIST);
#undef TheCheck

#define TheCheck(x) static void foo3##x(decltype(x) v){};
...
#define TheCheck(x, y)                       \
   ...
   foo1##x((decltype(x))y);
      ForFullmesh(TheCheck, NUMBER0_LIST);
#undef TheCheck
   }
}
...
#undef TheCheck
   ...
   Case(MOCK) {
#define TheCheck(x, y)                            \
   MOCK(foo2##x, void(decltype(x))).once(IsNull); \
   foo2##x((decltype(x))y);
...
#define ITERATOR_LIST      \
   int_array,              \
     const_int_array,      \
     intptr_t_array,       \
     const_intptr_t_array, \
     stdvector,            \
     const_stdvector

   Todo(OK ListOf) {
#define TheCheck(x) OK(ListOf(1, 2, 3), x);
      ForForEach(TheCheck, ITERATOR_LIST);
#undef TheCheck
   }
}
