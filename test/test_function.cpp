...
SUITE(function return void)
{
   ...
   Case(empty)
   {
      ...
      h2::h2_function<std::false_type, const Foo(int, std::string&)> f5;
      OK(f5.empty);

      h2::h2_function<Bar, const Foo(int, std::string&)> f6;
      OK(f6.empty);

      h2::h2_function<std::false_type, Foo&(int, Foo&)> f7;
      OK(f7.empty);

      h2::h2_function<Bar, Foo&(int, Foo&)> f8;
      OK(f8.empty);
   }
   ...
   Case(normal origin function)
   {
      ...
      h2::h2_function<std::false_type, const int(int, const char*)> f3;
   ...
   Case(normal origin member function)
   {
      ...
      h2::h2_function<Bar, Foo&(int, Foo&)> f8;
      f8.origin_function = (void*)ref_func;
      auto a8 = f8(&bar, 1, foo);
      OK(1, a8.int_value);
      OK("1", a8.char_value);
   }
