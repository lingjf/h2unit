...
SUITE(SOCK)
{
  ...
  Case(UDP sendto recvfrom)
  {
    ...
    int sent = sendto(
      sock, "1234567890", 10, 0, (struct sockaddr*)&remote, sizeof(remote));
    OK(10, sent);
    OK(PktEq("0.0.0.0:9527", "1.2.3.4:8888", Me("1234567890", 10), 10),
       H2SOCK()); // Fetch outgoing packet

    H2SOCK("9876543210",
           10,
           "4.3.2.1:4444"); // Inject as received packet from 4.3.2.1:4444
    l = sizeof(struct sockaddr);
    int r1 = recvfrom(
      sock, (char*)buffer, sizeof(buffer), 0, (struct sockaddr*)&c, &l);
    OK(10, r1);
    OK(Me("9876543210", 10), buffer);
    OK(IPeq("4.3.2.1:4444"), &c);

    H2SOCK("9876543210", 10); // Inject as received packet from last peer
    l = sizeof(struct sockaddr);
    int r2 = recvfrom(
      sock, (char*)buffer, sizeof(buffer), 0, (struct sockaddr*)&c, &l);
    OK(10, r2);
  ...
  Case(UDP sendmsg recvmsg)
  {
    ...
    OK(10, r2);
    OK(PktEq("*:9527", "1.2.3.4:8888", Me("1234567890", 10), 10), H2SOCK());
  ...
  Case(TCP server)
  {
    ...
    H2SOCK(
      "", 0, "1.2.3.4:8888"); // Inject as Connected Signal from 1.2.3.4:8888
    l = sizeof(c);
    ...
    H2SOCK("9876543210",
           10,
           "1.2.3.4:8888"); // Inject as received packet from 4.3.2.1:8888
  ...
  Case(TCP client)
  {
    ...
    H2SOCK("9876543210", 10, "1.2.3.4:8888");
