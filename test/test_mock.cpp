...
SUITE(mock c - function)
{
   ...
   Case(multi - line)
   {
      MOCK(bar1, int(int, const char*)).once(1, "A").returns(11).once().with(2, "B").returns(22).twice(3, "C").returns(33);
...
SUITE(mock does)
{
   ...
   {
      char t[128] = "hello";
      MOCK(bar2, void(int, char*)).once() = [](int a, char* b) {strcpy(b, "world"); return; };
      bar2(0, t);
...
SUITE(mock C++ Class member)
{
   ...
   Case(static member function)
   {
      ...
      MOCK(Shape::fly, int(int, int)).once() = [](int, int) -> int {
         return 22;
      };
      OK(22, Shape::fly(1, 2));
   ...
   Case(member function)
   {
      ...
      MOCK(Shape, go, int(int)).once(1) = [](int) -> int {
         return 22;
      };
      OK(11, shape.go(1, 2));
      OK(22, shape.go(1));
   ...
   Case(virtual member function lambda with this object pointer)
   {
      ...
      MOCK(Shape, work, int(int, int)).once(1, 2) = [](Shape* that, int x, int y) -> int {
         OK(that != nullptr);
         OK("333", that->id);
         OK(1, x);
         OK(2, y);
