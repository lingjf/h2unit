...
int h2unit_main(int argc, char **argv)
{
    ...
    int c;
    while (-1 != (c = getopt(argc, argv, "vbri:x:h?"))) {
        switch (c) {
        case 'v': __cfg._verbose = true; break;
        case 'b': __cfg._colored = false; break;
        case 'r': __cfg._random = true; break;
        case 'i': __cfg._include = optarg; break;
        case 'x': __cfg._exclude = optarg; break;
        case 'h':
        case '?':
        default:
            printf("Usage: \n"
                   "-v                  Make the operation more talkative\n"
                   "-b                  Output in black-white color mode\n"
